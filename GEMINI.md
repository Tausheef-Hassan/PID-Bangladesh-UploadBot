...
2.  **Image Processor:** 
    ...
    - **Automatic Archiving:** Submits the source image URL and detail page URL to the Wayback Machine without waiting for the capture; unconfirmed URLs are confirmed by the next run from a persistent queue.
    - Detects the photograph/caption boundary by scanning eight vertical edge columns bottom-up for where the flat page colour stops being flat; falls back to a background-row scan.
...
6.  **Post-Process Logging:** Updates a central Wikimedia data module (`Module:PIDDateData`) and logs execution results to the bot's daily JSON log page on Commons (one page per day, to stay under `$wgMaxArticleSize`).
...
## Key Files
- `main.py`: Pipeline orchestrator; the per-stage logic lives in `src/`.
- `src/`: `scraper.py`, `image_processor.py`, `translator.py`, `uploader.py`, `commons_log.py`, `wayback.py`.
- `config.py`: Constants, logging, the retrying HTTP session, IPv4 enforcement.
- `test_pipeline.py`, `test_processor_cv.py`: Offline `assert` self-checks; run them directly.
...
- **Resilience:** HTTP traffic goes through `config.http_session()`, a `requests.Session` with a `urllib3` `Retry` (exponential backoff with jitter, `429`/`5xx` and transport errors). Idempotent methods only, so Save Page Now `POST`s are never resubmitted automatically.
